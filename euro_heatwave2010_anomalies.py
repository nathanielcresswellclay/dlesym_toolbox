...
def main(config_path: str):
    ...
    """
    send config to routines that plot t2m and z500 anomalies during the 2010 heatwave in Europe
    ...
    logger.info("Finished plotting anomalies for the 2010 heatwave in Europe.")
...
def _plot_t2m_anoms(config: DictConfig , logger: logging.Logger):
    ...
    verif_temp = xr.open_zarr(config.verification_file).targets.sel(
        time=pd.date_range('2010-07-25', '2010-08-08', freq='6H'),
        channel_out="t2m0"
    )
    ...
    verif_temp = verif_temp.groupby("time.day").mean("time").rename({"day": "time"}).assign_coords(
        time=pd.date_range('2010-07-25', '2010-08-08', freq='D')
    )
    ...
    climatology = climatology.sel(
        dayofyear=pd.date_range('2010-07-25', '2010-08-08', freq='D').dayofyear
    ...
    for forecast in config.forecast_params: 
        ...
        for i, init in enumerate(config.inits):
            ...
            fcst_init = fcst_init.sel(time=pd.date_range('2010-07-25', '2010-08-08', freq='D')).groupby('time.day').mean('time').rename({'day': 'time'}).assign_coords(
                time=pd.date_range('2010-07-25', '2010-08-08', freq='D')
            )
