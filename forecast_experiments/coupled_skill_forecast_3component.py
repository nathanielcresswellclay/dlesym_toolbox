...
def main(config_path: str):
    ...
    # calculate forecast lead time from days to hours
    arg_template.lead_time = int(config.leadtime * 24) # leadtime in hours 
    arg_template.forecast_init_start =  config.init_time_start
    arg_template.forecast_init_end = config.init_time_end
    arg_template.freq = config.init_freq
    arg_template.gpu = 0
    ...
    for model_group in config.model_groups:
        ...
        arg.atmos_model_checkpoint = getattr(model_group, 'atmos_model_checkpoint', None)
        arg.ocean_model_path = model_group.ocean
        arg.ocean_dataset_path = model_group.ocean_dataset_path
        arg.ocean_model_checkpoint = getattr(model_group, 'ocean_model_checkpoint', None)
        arg.land_model_path = model_group.land
        arg.land_dataset_path = model_group.land_dataset_path
        arg.land_model_checkpoint = getattr(model_group, 'land_model_checkpoint', None)
        arg.output_directory = model_group.output_dir
        arg.cache_dir = model_group.output_dir
        arg.atmos_output_filename = f"atmos_{config.init_time_start.replace('-','.')}-{config.init_time_end.replace('-','.')}_{config.leadtime}day"
        arg.ocean_output_filename = f"ocean_{config.init_time_start.replace('-','.')}-{config.init_time_end.replace('-','.')}_{config.leadtime}day"
        arg.land_output_filename = f"land_{config.init_time_start.replace('-','.')}-{config.init_time_end.replace('-','.')}_{config.leadtime}day"
...
if __name__ == "__main__":
    ...
    # receive arguments 
    import argparse
    parser = argparse.ArgumentParser(description="Run coupled skill analysis for 3 components.")
