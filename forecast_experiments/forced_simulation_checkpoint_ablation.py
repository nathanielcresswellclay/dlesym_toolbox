...
def main(config_path: str):
    ...
    for ckpt in config.checkpoints:
        ...
        arg.model_path = config.model
        arg.output_directory = os.path.join(config.model, "forecasts/forced_checkpoint_ablation",)
        arg.output_filename = f"forecast_forced_{config.init_time.replace('-','.')}-{config.end_time.replace('-','.')}_{ckpt.replace('.mdlus','')}"
        ...
        arg.hydra_path = os.path.relpath(config.model, os.path.join(os.getcwd(), 'hydra'))
        arg.model_checkpoint = ckpt
        arg.batch_size = None
    ...
    for inference_arg in inference_args:
        ...
        if os.path.exists(forecast_file) and not config.overwrite_forecast:
