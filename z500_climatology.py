...
def _plot_global(data: xr.DataArray, mapper, output_file_prefix: str, diff_plot: bool=False):
    ...
    for season in data.season.values:
        ...
        data_ll_cyclic = data_ll_cyclic / 9.80665
        
        # Z500 in geopotential meters (gpm); adjust levels if your archive uses other scaling
        im = ax.contourf(
            lon_cyclic, data_ll.lat, data_ll_cyclic,
            transform=ccrs.PlateCarree(),
            cmap=_get_z500_cmap() if not diff_plot else _get_custom_cmap_bwr(),
            levels=np.arange(4800, 6100, 50) if not diff_plot else np.arange(-120, 125, 10),
            extend='both',
            zorder=5,
        )
        ...
        cbar.set_label(
            'Z$_{500}$ (m)' if not diff_plot else 'Z$_{500}$ difference (m)',
            fontsize=12,
        )
